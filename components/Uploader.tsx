...
const Uploader: React.FC<UploaderProps> = ({ onUpload, onUrlUpload }) => {
  ...
  return (
    <div className="w-full max-w-2xl space-y-10 animate-in fade-in zoom-in-95 duration-700">
      <div 
        className="flex flex-col items-center justify-center p-16 md:p-24 bg-[#1c1c1e]/40 rounded-[4rem] ios-shadow-lg border border-white/5 cursor-pointer active:scale-[0.99] transition-all group relative overflow-hidden"
      ...
      >
        ...
        <input 
          ...
          multiple
        />
        <div className="w-24 h-24 bg-[#007aff] rounded-[2.5rem] flex items-center justify-center mb-10 shadow-[0_20px_50px_rgba(0,122,255,0.4)] group-hover:scale-110 group-hover:rotate-6 transition-all duration-700 animate-float">
          <UploadIcon className="text-white w-12 h-12" />
        </div>
        <h3 className="text-4xl font-black tracking-tighter mb-4 text-white">Import Studio Assets</h3>
      ...
      <div className="p-4 bg-[#1c1c1e]/60 ios-blur border border-white/10 rounded-[3rem] flex items-center gap-4 shadow-2xl">
