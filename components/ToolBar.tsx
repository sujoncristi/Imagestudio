  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 p-6 md:p-10 flex justify-center z-50 pointer-events-none pb-safe">
      <div className="relative flex flex-col items-center max-w-full">
        {/* TOOLTIP */}
        <div className={`absolute -top-16 bg-white text-black text-[12px] font-black px-5 py-2.5 rounded-full shadow-2xl transition-all duration-500 pointer-events-none uppercase tracking-[0.2em] ${hoveredTool ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-4 scale-90'}`}>
          {tools.find(t => t.id === hoveredTool)?.tooltip}
        </div>

        {/* TOOLBAR BODY */}
        <div className="bg-[#1c1c1e]/80 ios-blur border border-white/10 rounded-[3rem] p-3 flex items-center gap-2 max-w-[95vw] overflow-x-auto no-scrollbar pointer-events-auto shadow-[0_40px_80px_rgba(0,0,0,0.6)]">
          {tools.map((tool) => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-[9px] font-black uppercase tracking-tighter ${isActive ? 'text-black' : 'text-white/20 group-hover:text-white/40'}`}>
                  {tool.label}
                </span>
              </button>
            );
